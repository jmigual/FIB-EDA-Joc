...
struct PLAYER_NAME : public Player {
    ...
    struct find {
        ...
        static find makeFind(int a, int b, const Posicio &pos) { return find(a, b, pos); }
    };
    ...
    bool passar(Posicio &p) {
        ...
        return que != MUNTANYA && que != AIGUA && temps_foc(p.x, p.y) < MAX_FOC;
    }
            ...
            if (atac[p.second.x][p.second.y] == NO_VIST) {
                ...
                if (!passar(p.second)) {
                    ...
                    if (p.first == POST) {
                        ...
                        for (int i = 0; i < DIRS; ++i) {
                            Q.push(PEP(INV[i], Posicio(p.second.x + X[i], p.second.y + Y[i])));
                        }
                    }
                }
            }
    ...
    void updatePosts() {
        dir = VVPa(MAX, VPa(MAX, {NO_VIST, NO_VIST}));
        ...
        while (!Q.empty()) {
            ...
            if (dir[p.pos.x][p.pos.y].first == NO_VIST) {
                ...
                if (que == MUNTANYA || que == AIGUA || temps_foc(p.pos.x, p.pos.y) >= MAX_FOC) {
                    ...
                    if (p.dir == POST)
                        ...
                        dir[p.pos.x][p.pos.y] = P(p.dir, p.dist);

                    // Per cada casella al voltant afegim
                    for (int i = 0; i < DIRS; ++i) {
                        Q.push(find(INV[i], p.dist + 1, Posicio(p.pos.x + X[i], p.pos.y + Y[i])));
                    }
                }
            }
        }
        ...
        if (quin_torn() % 30 == 0) {
            for (int i = 0; i < MAX; ++i) {
                ...
                for (int j = 0; j < MAX; ++j) {
                    if (dir[i][j].first >= 0)
                        std::cerr << dir[i][j].first << "  ";
                    else if (dir[i][j].first == NO_PASSAR)
                        std::cerr << "N  ";
                    else if (dir[i][j].first == POST)
                        std::cerr << "P  ";
                    else
                        std::cerr << "V ";
                }
    ...
    virtual void play() {
        ...
        if (quin_torn() == 0) {
            ...
            for (std::pair<const int, int> &a : pers)
