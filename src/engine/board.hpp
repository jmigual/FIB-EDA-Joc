...
const int MAX = 60; // Board size

const int NUM_SOLDATS = 20; // Initial number of soldiers per team
...
struct Posicio {
    ...
    bool operator!=(const Posicio &p) const { return x != p.x || y != p.y; }
};

// Retorna si (x,y) és en els límits del tauler.
inline bool valid(int x, int y) { return 0 <= x && x < MAX && 0 <= y && y < MAX; }

// Retorna si p és en els límits del tauler.
inline bool valid(const Posicio &p) { return valid(p.x, p.y); }
...
struct Post {
    ...
    bool operator!=(const Post &p) const {
        return equip != p.equip || pos != p.pos || valor != p.valor;
    }
};
...
struct Info {
    ...
    std::vector<int> paraca; // Vector de comptadors de temps dels paracaigudistes, ordenats de
                             // petit a gran. Com més petit és el comptador, menys torns té el
                             // paracaigudista per saltar. (buit si soldat).
    ...
    Info() : id(0), tipus(0), equip(0), pos(0), vida(0), orientacio(0), napalm(0), paraca() {}

    bool operator!=(const Info &i) const {
        return id != i.id || tipus != i.tipus || equip != i.equip || pos != i.pos || vida != i.vida
               || orientacio != i.orientacio || napalm != i.napalm || paraca != i.paraca;
    }
};

// Info que es retorna per marcar que algunes comandes han estat errònies.
const Info NUL(-1, -1, -1, -1, -1, -1, -1, -1, std::vector<int>());

typedef std::vector<atzar> VA;
typedef std::vector<int> VE;
typedef std::vector<VE> VVE;
typedef std::vector<VVE> VVVE;
typedef std::vector<Posicio> VP;
typedef std::vector<VP> VVP;
typedef std::vector<Post> VP2;
typedef std::map<int, Info> MEI;
typedef MEI::iterator MI;
typedef MEI::const_iterator MIc;
typedef std::set<int> SE;
typedef std::map<int, Posicio> MEP;
typedef std::pair<int, Posicio> P2;
...
public:
    ...
    std::vector<int> soldats(int e) const;

    // Retorna els identificadors dels helicòpters de l'equip e.
    // Retorna un vector buit si l'equip és incorrecte.
    std::vector<int> helis(int e) const;
    ...
    //    * orientacio  (cap a on mira l'helicòpter: 0 -> S, 1 -> E, 2 -> N, 3 -> W, -1 per als
    //    soldats),
    //    * napalm      (torns que falten fins a tenir napalm disponible, -1 per als soldats).
    //
    // El mètode dades(id) retorna NUL (una constant predefinida de tipus Info, amb tots els camps a
    // -1) si no hi ha cap efectiu amb aquest identificador.
...
private:
    ...
    std::vector<double> status_; // cpu status. <-1: dead, 0..1: %of cpu time limit
    ...
    void propaga_foc();
    void fes_torn_soldat(int id, int x2, int y2, std::vector<Action> &done);
    void fes_torn_soldats(std::vector<P2> &V, std::vector<Action> &done);
    void fes_torn_heli(int id, int ins, std::vector<Action> &done);
    void fes_torn_helis(std::vector<P> &ordres2, std::vector<Action> &done);
    void paracaigudistes_baixen(const VVP &ordres3, std::vector<Action> &done);
    ...
    Board next(const std::vector<Action> &asked, std::vector<Action> &done) const;
...
inline int Board::qui_soc() const { return jugador; }

inline std::string Board::nom(int e) const {
    if (e < 1 || e > 4) {
...
inline int Board::quin_torn() const { return torn; }

inline int Board::torns_restants() const { return TORNS_JOC - torn; }
...
inline std::vector<int> Board::soldats(int e) const {
    if (e < 1 || e > 4) {
...
inline std::vector<int> Board::helis(int e) const {
    if (e < 1 || e > 4) {
...
inline std::vector<Post> Board::posts() const { return post; }
...
inline std::vector<int> Board::permutacio(int n) {
    assert(es_jugador());
    if (n < 0) {
...
inline bool Board::es_jugador() { return jugador >= 1 && jugador <= 4; }

inline int Board::nou_id() { return nou++; }
...
inline void Board::propaga_foc() {
            ...
            if (foc[x][y]) {
                ...
                if (terreny[x][y] == BOSC) {
                    ...
                    for (int k = 0; k < 4; ++k) {
                        int xx = x + Xcreu[k];
                        int yy = y + Ycreu[k];
                        if (terreny[xx][yy] == BOSC && foc[xx][yy] == 0)
                            B[xx][yy] = true;
                    }
                }
            }
    ...
    for (int x = 0; x < MAX; ++x)
        for (int y = 0; y < MAX; ++y)
            if (B[x][y] && A.probabilitat(PROB_FOC)) {
                foc[x][y] = CREMA_BOSC;
