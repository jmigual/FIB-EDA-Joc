...
struct Post {
    ...
    Post(int equip, int x, int y, int v) : equip(equip), pos(Posicio(x, y)),
        valor(v) { }
...
struct Info {
    ...
    vector<int>
    paraca;  // Vector de comptadors de temps dels paracaigudistes, ordenats de petit a gran.
    ...
    Info(int id, int tipus, int equip, int x, int y, int vida, int orientacio,
         int napalm, const vector<int> &paraca) : id(id), tipus(tipus), equip(equip),
        pos(x, y), vida(vida), orientacio(orientacio), napalm(napalm),
        paraca(paraca) { }
